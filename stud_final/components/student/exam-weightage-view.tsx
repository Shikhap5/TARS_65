      ...
      <CardContent className="space-y-6">
        ...
        <div className="bg-accent/10 border border-accent/30 rounded-lg p-4">
          ...
          <p className="text-sm text-foreground/80">
            Focus on <strong>{subjects.reduce((a, b) => (a.examWeightage > b.examWeightage ? a : b)).name}</strong> as
            it has the highest weightage ({Math.max(...subjects.map((s) => s.examWeightage))}%). Allocate your study
            time proportionally to each subject's weightage for optimal preparation.
          </p>
        </div>
      </CardContent>
    </Card>
